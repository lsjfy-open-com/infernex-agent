> 当前功能与跨平台演进见[文档入口](docs/README.md)及[通用底座能力矩阵](docs/architecture/kubernetes-first-zh.md)。alpha.18 的原生部署写路径和请求级均衡尚未实现。

# InferNex Agent

alpha.18 延续 alpha.16 的可选 SLO 对照实验与 Git / snapshot 版本关联记录，并修复 Host Dashboard 直访和原生 Kubernetes 空页面问题；使用方式和能力边界见[SLO 与配置版本指南](docs/guides/slo-and-config-versions-zh.md)。
...
```bash
curl -fsSL https://raw.githubusercontent.com/lsjfy-open-com/infernex-agent/infernex-agent-v0.5.0-alpha.18/component/InferNex-Agent/scripts/install.sh | sudo env INFERNEX_AGENT_VERSION=0.5.0-alpha.18 bash
```
...
当前 Pi TUI 默认入口测试包为 [`v0.5.0-alpha.18`](https://github.com/lsjfy-open-com/infernex-agent/releases/tag/infernex-agent-v0.5.0-alpha.18)（继承 alpha.12 功能并整合部署/恢复保护），同时提供 amd64 与 arm64，不替换当前 v0.4 RC 稳定测试线。alpha.12 包含默认 8192 输出上限、跨 Session 语义记忆、默认折叠 reasoning block，以及受控的历史日志分析与 Markdown 报告。完整离线包也已内置固定版本的 `ripgrep (rg)` 和 `fd`，TUI 文件搜索不再依赖目标服务器联网安装工具。
...
alpha.18 新安装的 Dashboard 默认监听 `0.0.0.0:8081`，浏览器访问 `http://<管理节点 IP>:8081/`，无需 SSH 隧道。MCP 和诊断子代理继续默认监听本机回环地址。升级时保留已有 Dashboard 地址；旧安装若仍为 `127.0.0.1`，需显式配置 `--dashboard-listen-address 0.0.0.0:8081`。
