...
#### 3.2.1 DeepSeek Harness：自组织、候选构建与稳定发布的关系

2026-09-23 核对的官方资料表明：DeepSeek Harness 基于 Cordis 插件体系，能力可通过配置组合；Creator 模式支持运行时检查、内存中的插件试验和新模式构建。这支持“Agent 可参与构建和调整自己的工作环境”的理解，尚不能据此认定它会自动识别 Kubernetes 业务语义、推导完整本体或验证生产适配器。[官方介绍](https://deepseek.com/harness/en/)

工具注册有明确的输入/输出契约和执行函数；Skill 通过 provider 注册、发现和加载；subagent 也通过独立 provider 与委派工具组织。它们分别是执行接口、知识指导和任务执行者，不是同一种“元数据”。外部 MCP server 的接入也是配置与作用域管理的一部分。[工具契约](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/subsystems/tools.md)、[Skill 注册](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/subsystems/skills.md)、[subagent 机制](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/subsystems/subagent.md)、[MCP 接入](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/subsystems/mcp.zh.md)

这里要区分三种元数据：**环境元数据**来自集群观测与资料核对；**能力元数据**描述工具 schema、Skill 适用条件、插件依赖，可以由开发者或 Agent 起草并校验；**运行元数据**记录本次启用版本、任务委派、权限与执行轨迹。自组织主要改变能力选择与编排，不表示环境事实能由模型自行生成，也不表示新插件天然可靠。

因此，本课题采纳的是**在领域约束下构建、验证并沉淀能力**，不要求立即替换现有 Pi/Go 运行时。以下是我们提出的设计，不是 DeepSeek 官方已经实现的 Kubernetes 功能：

| 层次 | 可以自组织/构建的部分 | 必须保持稳定的约束 |
| --- | --- | --- |
| 环境事实 | 采集后更新组件、版本和依赖关系，补充证据 | 客户对象原始身份、来源与时间不可被模型猜测覆盖 |
| 任务编排 | 按故障拆分网络、引擎、版本核对 subagent，选择工具和 Skill | 任务预算、权限、父子关系、共享工作负载写锁、取消传播和结果归属 |
| 能力候选 | 针对未知框架生成映射、MCP 包装、Skill 或 subagent preset 草稿 | 输入/输出 schema、适用版本、资源管理权、批准与回退合同 |
| 稳定组件 | 对通过测试的候选固化版本、摘要、依赖和适用矩阵 | 未通过独立验收的代码不能进入生产工具目录；升级可撤销 |

领域模型与 Harness 是互补的：模型表达“什么对象、关系和操作有效”，Harness 管理“由谁、用什么能力、怎样持续执行”。无需先完成一套包罗万象的本体再开始构建；从最小规则出发，以真实案例迭代，候选发现的新概念经过审阅再进入 schema。运行轨迹用于审计执行，不代替组件依赖图、现场快照或物理故障证据。内存中的插件试验不等于与生产环境隔离，候选构建应在不持有生产写凭据的独立实验环境完成。

```mermaid
flowchart TD
  A[领域基本规则 + API 说明 + 现场证据] --> B[能力缺口]
  B --> C[构建 subagent 生成候选工具/Skill/适配映射]
  C --> D[隔离构建、静态检查与合同测试]
  D --> E[独立评估者使用保留用例验收]
  E --> F{达标且影响范围明确?}
  F -- 否 --> G[退回修改或保留未知状态]
...
新增 B11 验收建议：出题方提供 2 类开发用封装框架和至少 1 类保留框架，每类都交付 schema、脱敏实例、管理语义、读写权限、API mock 与恢复合同；仅保留测试的答案和故障注入真值不交给构建 Agent。候选至少经历参数无效、缺权限、重复执行、并发漂移、取消和恢复 6 类负例，全部通过才可进入稳定目录；保留框架至少完成一个独立部署/撤销场景，连续 3 次成功，未批准写入与越权均为 0。验证 subagent 无权限放大、取消后无遗留写任务，且同一工作负载变更串行化。

对照固定工具方案、仅运行时装配方案和允许候选构建方案，冻结模型、任务、token/时间预算，报告每例成功率、总耗时、人工介入、模型调用成本、候选失败和回归数；构建费用计入总成本。开发集与保留集分开，不得将答案放进 Skill。至少 1 类保留框架完成受控扩展是本期门槛；“更适合泛化、成本更低”必须由对照实测支持，不预先宣称结论。环境版本和能力包变化后重新匹配，不能把一次生成成功当成永久适用。
...
实施者不负责猜测测试环境、补齐未提供的真值或自行找到客户硬件。具体起步命令、文件位置、数据生成/校验、采集及清理步骤见[验收实验室搭建与数据指南](https://github.com/lsjfy-open-com/infernex-agent/blob/develop/component/InferNex-Agent/docs/guides/next-generation-acceptance-lab-zh.md)，可执行起步材料见[验收包目录](https://github.com/lsjfy-open-com/infernex-agent/tree/develop/component/InferNex-Agent/test/acceptance/next-generation)。

| 出题方应交付 | 必须描述的内容 | 本次材料与就绪边界 |
| --- | --- | --- |
| CPU 控制逻辑实验室 | Host 规格、工具版本、Kind 拓扑、固定资源清单、安装/校验/清理命令 | 提供起步配置与指南；pause 容器只用于资源识别，不能承接推理或吞吐验收 |
| 案例和数据 | 案例目标、可见输入、期望证据、隐藏真值、评分规则、hash 和许可证/授权 | 提供三类确定性合成时间线与校验程序；用于测试数据管线和诊断规则，不冒充真实 plog、HCCL 或交换机日志 |
| Bridge 和客户封装环境 | 实际控制器及版本、CRD/API、批准模板、实例、凭据分发与管理语义 | 复用现有 Bridge 自动测试说明；真实客户框架和 B11 保留用例尚需出题方提供，贴 Helm 标签不等于真实 Helm 管理 |
| GPU/NPU/Mooncake 现场 | 实际设备/驱动/通信库/引擎版本及安装包摘要、TP/节点布局、模型挂载、内网入口、配额与测试窗口 | 提供填写与预检合同；不能从当前仓库推断具体可用机器、镜像或性能，填齐且预检通过后才启动硬件验收 |
| 联网/断网供应清单 | 基础与目标镜像及依赖、内网模型和仓库、外部查询许可、出口阻断证据、导入渠道 | 提供准备步骤；真正离线验收需要目标环境预置全部依赖，不能只屏蔽测试 Pod 网络后宣称 Host 也已隔离 |
| 实测报告与评分器 | 每项输入/输出字段、阈值、基线测量、失败/缺证规则、原始日志和校验值 | 提供结果模板；未跑过的项目记录未测，正式出题不能以空模板代替基线实测报告 |

硬件日志至少包括：统一 UTC 时间和时钟偏差估计、request/trace ID、实例与 TP rank、prefix 命中证据、排队/提交/传输/远端完成事件、参与节点应用日志、可授权采集的 plog/通信日志，以及同时间窗 PFC/RDMA/网卡与交换机端口计数器增量。拿不到的字段写明 unavailable 和原因，不能补造；局部样本不直接拼成可靠跨机时序。

出题方先校验已知正常和已知异常基线各一次，保证注入可撤销、请求可复现、日志能关联，再交付实施者。真实数据脱敏保持 ID 之间的关联，提供脱敏规则和来源授权；删除凭据而不改变故障时序。开发数据可公开真值，正式盲测使用未公开变体，将评估者答案放在另一账号/环境，不能仅靠不同目录名隔离。公网仓库内的样例不充当隐藏评测集。

**开测门槛**：出题方与验收方共同签收环境 manifest、数据清单和基线报告；未提供硬件、补丁依赖或客户框架的用例标记“未就绪”，不向实施者宣称可复现，也不将其缺失算作实现失败。当前起步包不代表全部 A/B 用例已经具备运行条件。
...
| B11 受约束的能力构建 | 2 类开发框架、至少 1 类保留框架；构建工具/Skill/适配映射候选并固化能力包 | 按 3.2.1 的 6 类负例全部通过；保留框架部署/撤销连续 3 次成功，未经批准写入和权限放大均为 0；固定/装配/构建三方案同预算对照，完整计入构建成本。公开合成样例不算保留测试 |
...
能力建设补充 B11：在隔离环境生成适配器、工具、Skill 与 subagent preset 候选，经独立合同测试和保留用例验证后固化；与 B2 的运行时装配分别计量。可以借鉴 DeepSeek Harness 的插件组织方式，不将全面迁移运行时作为前置条件。
