...
| E2 修复与补丁制品 | 隔离构建、固定基础镜像、补丁/组合版本清单、离线交付与实验恢复；以 Git 期望配置、变更前后 snapshot 和 ReleaseManifest 关联版本 | 待实现；一次修复可重装、复验、撤销，能从版本 ID 找到配置提交、现场快照及制品，不把 Agent 安装包等同服务补丁 |
...
E2a/E2b 是 E2 的知识来源增量验收，不改变 E2 原有的构建、离线交付和恢复范围。完整组合版本管理仍属 E2 规划；当前只有 Bridge 的 InferNexService 状态、变更记录与受管候选回退。
