...
- exec、SSH、宿主机文件不混入通用 API 读取，但可作为 `diagnose` 及以上模式的受控通道；固定只读探针属于 active-read，任意 shell 永不作为模型参数；
...
- 固定禁用 Pi 内置 `bash/read/write/edit` 等 coding tools，模型不能绕过 MCP；需要 Pod exec、宿主机或 SSH 时只能调用 Core 发布的 typed active-read 工具；
...
- **执行通道不等于修改权限**：Pod exec、宿主机程序和预配置 SSH alias 可承载固定的 NPU/CANN/网络
  只读探针；模型不能提交命令字符串。是否允许由 mode、action class、target scope、预算和审批共同决定；
