...
`infernex-agent cluster-state verify` ??????????
...
| ?????? | ???????? | ?????????? |
| ?????? | ?????????? | ????????????????? |
| ???? waiting-ready | ?????NPU ???Bridge Event | ????/??????????????? |
| ????????? | comparison ? incident ??? | ??? NPU/HCCL/Mooncake ??????????? stream ?? |
| ???????? | ??/?????????? | ??????????????????/?????? |
...
`--purge-state` ??????????? InferNex ?????ServiceAccount?Role/RoleBinding ? Token
