 ...
 return {
  ...
  renderCall(args, _theme, context) {
   ...
   return { invalidate() {}, render(width: number) {
    ...
    const target = [input.namespace, input.pod || input.name || input.title || input.subject || input.sshTarget || input.probe].filter(v => typeof v === "string").join("/");
